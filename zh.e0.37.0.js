...
/* 发布闸门文案（r71 P0 修复）：此前 release_gate === "DENY" 时反被显示成「正式版」，
   与真实的 RELEASE_GATE=DENY / LANDED=FALSE 相反，属于对外错误宣称。这里只认 ALLOW，
   其余一切取值一律按「未发布」显示，避免再次出现「闸门拒绝却写着正式版」。 */
const RELEASE_GATE_ALLOW = "ALLOW";
function releaseGateWord(value) {
  return String(value || "").trim().toUpperCase() === RELEASE_GATE_ALLOW ? "正式版" : "内测中（未发布）";
}
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (body && body.algorithm_version) {
        CHANNEL = "api";
        const elAlgo = $("st-algo"); if (elAlgo) elAlgo.textContent = body.algorithm_version;
        const elAuth = $("st-authority"); if (elAuth) elAuth.textContent = body.production_authority === "OFF" ? "决策支持" : body.production_authority || "决策支持";
        const elGate = $("st-gate"); if (elGate) elGate.textContent = releaseGateWord(body.release_gate);
        const elChan = $("st-channel"); if (elChan) elChan.textContent = "云端计算";
        const elChanHint = $("st-channel-hint"); if (elChanHint) elChanHint.textContent = "已连接云端计算服务";
        setEnginePhase("api");
...
function setBrowserChannel() {
  CHANNEL = "browser";
  const elChan = $("st-channel"); if (elChan) elChan.textContent = "浏览器内引擎";
  if (typeof renderArchiveChannelNotice === "function") renderArchiveChannelNotice();
  const elChanHint = $("st-channel-hint"); if (elChanHint) elChanHint.textContent = "浏览器内计算引擎（无需等待，后台自动准备）";
  // 安全状态必须来自引擎本体，不能写死在页面里；引擎就绪后再填充。
  const elAlgo = $("st-algo"); if (elAlgo) elAlgo.textContent = "准备中…";
  const elAuth = $("st-authority"); if (elAuth) elAuth.textContent = "准备中…";
  const elGate = $("st-gate"); if (elGate) elGate.textContent = "准备中…";
}
...
async function ensureBrowserEngine() {
  ...
  const run = (async () => {
    ...
    $("st-gate").textContent = releaseGateWord(meta.release_gate);
...
function collectProfile() {
  ...
  const educationTargetYear = $("w-edu-target-year").value.trim() === "" ? null : $("w-edu-target-year").value.trim();
  const education_plan = {
    beneficiary: $("w-edu-child").value.trim(),
    target_year: educationTargetYear,
    years_until_target: educationTargetYear === null ? null : (Number(educationTargetYear) - new Date().getFullYear()),
...
function validate() {
  ...
  /* 子女教育资金计划（可选，但填了就必须可计算、可复核） */
  const edu = profile.education_plan || {};
  const eduAny = Object.values(edu).some((v) => v !== null && v !== undefined && v !== "");
  if (eduAny) {
    if (edu.target_year == null && edu.years_until_target == null) {
      problems.push("子女教育资金计划填了内容，但缺少目标年份。");
    } else if (edu.years_until_target != null && Number(edu.years_until_target) < 0) {
      problems.push("子女教育资金计划的目标年份早于今年：目标已过，请把目标年份改到未来。");
    }
    if (edu.expected_cost == null || Number(edu.expected_cost) < 0) {
      problems.push("子女教育资金计划需要预期费用总额（非负数字），引擎才能计算缺口。");
    }
    if (edu.inflation_assumption == null || Number(edu.inflation_assumption) < 0 || Number(edu.inflation_assumption) > 1) {
      problems.push("子女教育资金计划的通胀假设必须填 0–1 之间的小数（例如 0.03）。");
    }
    if (edu.return_assumption == null || Number(edu.return_assumption) < 0 || Number(edu.return_assumption) > 1) {
      problems.push("子女教育资金计划的回报假设必须填 0–1 之间的小数（例如 0.05）。");
    }
    if (edu.current_savings != null && Number(edu.current_savings) < 0) {
      problems.push("子女教育资金计划的现有储备不能为负。");
    }
    if (edu.monthly_investment != null && Number(edu.monthly_investment) < 0) {
      problems.push("子女教育资金计划的每月投入不能为负。");
    }
  }
...
/* 子女教育资金计划缺口（edu_plan_v1）：把家庭自己记录的教育计划（目标年份、现有储备、
   预期费用、通胀与回报假设、每月投入）渲染成可复核的缺口与进度。页面只展示引擎算出的
   数字与公式原文，不做任何二次估算。 */
function renderEducationPlanBlock(result) {
  const ep = (result || {}).education_plan || {};
  if (!ep || typeof ep !== "object" || !ep.status) return "";
  if (ep.status === "NOT_RECORDED") {
    return `
      <h3 style="margin:14px 0 8px;font-size:14px">子女教育资金计划</h3>
      <div class="note">${esc(ep.finding_cn || "家庭未记录子女教育资金计划。")}</div>`;
  }
  if (ep.status === "REFUSED") {
    return `
      <h3 style="margin:14px 0 8px;font-size:14px">子女教育资金计划</h3>
      <div class="note bad">${esc(ep.finding_cn || "教育资金计划信息不全，引擎拒绝给伪精确结论。")}</div>`;
  }
  const cls = ep.status === "ON_TRACK" ? "ok" : "bad";
  const row = (k, v, h = "") => `<div class="metric"><div class="k">${k}</div><div class="v">${esc(v)}</div>${h ? `<div class="h">${esc(h)}</div>` : ""}</div>`;
  const scenarios = (ep.scenarios || []).map((sc) =>
    `<tr><td>${esc(sc.label_cn)}<div class="subnote">${esc(sc.note_cn || "")}</div></td>
     <td class="num">${money(sc.future_cost)}</td><td class="num">${money(sc.projected_funds)}</td>
     <td class="num ${sc.gap > 0 ? "neg" : "pos"}">${money(sc.gap)}</td>
     <td class="num">${sc.progress_pct === null || sc.progress_pct === undefined ? "—" : fmt(sc.progress_pct, 1) + "%"}</td></tr>`).join("");
  const who = [ep.beneficiary, ep.target_year ? `${ep.target_year} 年` : ""].filter(Boolean).join(" · ");
  return `
    <h3 style="margin:14px 0 8px;font-size:14px">子女教育资金计划（缺口 ${esc(ep.status_cn || "")}）</h3>
    <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:8px">
      <span class="pill ${cls}">${esc(ep.status_cn || ep.status)}</span>
      ${who ? `<span class="pill">${esc(who)}</span>` : ""}
      <span class="pill">目标年费用 ${money(ep.future_cost)}</span>
      <span class="pill">预计资金 ${money(ep.projected_funds)}</span>
      <span class="pill">缺口 ${money(ep.gap)}</span>
      ${ep.progress_pct === null || ep.progress_pct === undefined ? "" : `<span class="pill">准备进度 ${fmt(ep.progress_pct, 1)}%</span>`}
    </div>
    <div class="note">${esc(ep.finding_cn || "")}</div>
    <div class="note mono" style="font-size:12px">${esc(ep.formula_cn || "")}</div>
    ${scenarios ? `<div class="scroll" style="margin-top:8px"><table><thead><tr><th>情景</th><th class="num">目标年费用</th><th class="num">预计资金</th><th class="num">缺口</th><th class="num">准备进度</th></tr></thead><tbody>${scenarios}</tbody></table></div>` : ""}`;
}
...
function renderBreadwinner(result) {
  ...
  $("r-breadwinner-education").innerHTML = `
    ...
    ${renderEducationPlanBlock(result)}`;
...
function renderHomeDashboard() {
  ...
  /* r65：首页默认只有统一对话；总览只在真的有档案或结果时出现，不摆空占位。 */
  if (dash) dash.classList.toggle("hidden", !(has || (src && src.result)));
  if (!has && !(src && src.result)) {
    if (statusEl) statusEl.textContent = "还没有家庭档案。对 Verity 说一句「建立我的家庭档案」开始。";
  ...
  const eduPlan = r.education_plan || {};
  const eduPlanTxt = eduPlan.status && eduPlan.status !== "NOT_RECORDED"
    ? ` · 计划缺口 ${money(eduPlan.gap)}（${eduPlan.status_cn || eduPlan.status}）`
    : "";
  const eduHint = `教育资金需求 ${money(edu.need)} · 状态 ${edu.status || "—"}${edu.funded_events ? ` · ${edu.funded_events.length} 个情景可隔离` : ""}${eduPlanTxt}`;
  setHomeMetric(
    "education",
    edu.funded_events && edu.funded_events.length > 0 ? "已隔离" : "未隔离",
    eduHint,
    (edu.unfunded_events && edu.unfunded_events.length > 0) || (eduPlan.status === "GAP") ? "neg" : "pos"
  );
  setHomeMetric(
    "protection_gap",
...
function bind() {
  ...
  if (esStart) esStart.addEventListener("click", () => {
    ensureBrowserEngine().catch(() => {});
  });
  const esRetry = $("es-retry");
  if (esRetry) esRetry.addEventListener("click", retryBrowserEngine);
  ...
  if (askForm) {
    askForm.addEventListener("submit", (event) => {
      event.preventDefault();
      askVerityRender($("ask-input").value);
    });
  }
  document.querySelectorAll("#ask-chips button").forEach((btn) => {
    btn.addEventListener("click", () => {
      askVerityRender((btn.dataset.q || "").trim());
    });
  });
  bindHomeJump();
  renderHome();
...
function registerServiceWorker() {
  ...
  navigator.serviceWorker.register("./sw.js?v=e0.37.0")
    .then((registration) => {
...
/* 直接问 Verity · 真实 AI 通道（r72）：把「问题 + 本次引擎结果摘要」发到同源 /api/ask，
   由服务端带密钥调用上游模型；服务端拿不到档案密文，只拿得到这几个已算出来的数字。
   任何一步不成立（未登录 / 未配置 / 网络失败 / 超时）都退回上面的本地规则回答，
   并在回答前面如实标明这一条到底是谁答的 —— 本地回答绝不冒充 AI。 */
function askLanguage() {
  const declared = String((document.documentElement && document.documentElement.lang) || "").toLowerCase();
  if (declared.indexOf("en") === 0) return "en";
  if (declared.indexOf("yue") >= 0 || declared.indexOf("zh-hk") >= 0) return "yue";
  return "zh";
}

function askContextFromResult(r) {
  const trend = (r && r.trend) || {};
  const summary = trend.summary || {};
  const safety = (r && r.safety) || {};
  const today = (r && r.today) || {};
  const holder = (r && r.breadwinner) || {};
  const gap = holder.protection_gap || {};
  const edu = holder.education || {};
  const num = (value) => {
  ...
  const ctx = {
    currency: text(summary.currency) || "CNY",
    lang: askLanguage(),
  };
  const put = (key, value) => {
    if (value !== undefined && value !== null) ctx[key] = value;
  };
  put("safety_days", num(today.safety_days) === undefined ? num(safety.safety_days) : num(today.safety_days));
  put("debt_total", num(summary.total_debt_end));
  put("debt_pressure_ratio", num(safety.debt_pressure_ratio));
  put("protection_gap", num(gap.gap_after_insurance));
  put("protection_worst_event", text(gap.worst_event_label_cn));
  put("education_need", num(edu.need));
  put("education_status", text(edu.status));
  put("net_worth_start", num(summary.net_worth_start));
  put("net_worth_end", num(summary.net_worth_end));
  put("net_worth_change", num(summary.net_worth_change));
  if (Array.isArray(summary.deficit_months)) put("deficit_months", summary.deficit_months.length);
  put("today_action_cn", text(ACTION_CN[today.action]));
  put("today_action_reason", text(today.action_reason));
  put("annual_goal", text((r && r.annual_goal && r.annual_goal.state_label_cn) || ""));
  return ctx;
}

async function askVerityAI(question) {
  const src = homeResult();
  const answer = $("ask-answer");
  if (!answer) return { ok: false, reason: "no_element" };
  if (!src || !src.result) return { ok: false, reason: "no_result" };
  const txt = String(question || "").trim();
  if (!txt) return { ok: false, reason: "no_question" };
  ...
  try {
    const res = await fetch(channelUrl("api/ask"), {
      method: "POST",
      headers: { "content-type": "application/json", Accept: "application/json" },
      body: JSON.stringify({ question: txt, context: askContextFromResult(src.result) }),
      credentials: "same-origin",
      signal: controller ? controller.signal : undefined,
    });
    if (!res || !res.ok) return { ok: false, reason: res ? `http_${res.status}` : "no_response" };
    const body = await res.json();
    const text = body && typeof body.answer === "string" ? body.answer.trim() : "";
    if (!text) return { ok: false, reason: "empty_answer" };
    return { ok: true, answer: text, model: body.model || "" };
  } catch (err) {
...
function askVerityRender(question) {
  const answer = $("ask-answer");
  if (!answer) return;
  askVerityAI(question).then((res) => {
    if (res && res.ok) {
      answer.textContent = `（AI 回答 · 基于你最近一次家庭结果）\n${res.answer}`;
      return;
    }
    /* 降级路径必须保留完整可用性：AI 不通时仍然是原来那套本地规则回答。 */
    askVerity(question);
    if (answer.textContent) {
      answer.textContent = `（本地回答 · AI 服务暂不可用）\n${answer.textContent}`;
    }
  });
}
